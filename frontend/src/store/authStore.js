...
export const useAuthStore = create((set) => ({
  ...
  signup: async (email, password, name) => {
    ...
    try {
      const response = await axios.post(`${API_URL}/sign-up`, { email, password, name }, { withCredentials: true })
      set({
  ...
  socialLoginCallback: async (provider, token) => {
    set({ isLoading: true, error: null, message: null })
    try {
      const response = await axios.post(
        `${API_URL}/social-login/callback`,
        {
          provider,
          token,
        },
      ...
      set({
        isLoading: false,
        error: error?.response?.data?.message || error.message || `Error logging in with ${provider}`,
      })
      throw error
    }
  },
